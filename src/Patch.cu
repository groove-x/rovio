...
__global__ void extractPatchFromImageKernel(
    ...
    const int patchSize, const float c_x, const float c_y,
    const bool withBorder) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  int halfpatch_size = patchSize / 2 + (int)withBorder;

  // Calculate the coordinates in the image
  float img_x = c_x + x - halfpatch_size;
  float img_y = c_y + y - halfpatch_size;

  // Check if the thread is within the patch bounds
  if (x < 2 * halfpatch_size && y < 2 * halfpatch_size && img_x >= 0 &&
      img_y >= 0 && img_x < d_img.cols && img_y < d_img.rows) {
    // Perform the patch extraction
    int img_x_int = static_cast<int>(img_x);
    int img_y_int = static_cast<int>(img_y);
    d_patch[y * patchSize + x] = d_img(img_y_int, img_x_int);
  }
}
#endif
...
__global__ void extractPatchFromImageKernel(
  ...
  int x_wo_border = x - 1;
  int y_wo_border = y - 1;
  bool within_border = x_wo_border >= 0 && y_wo_border >= 0 &&
                       x_wo_border < 2 * (halfpatch_size - 1) &&
                       y_wo_border < 2 * (halfpatch_size - 1);

  // Check if within bounds
  if (x < 2 * halfpatch_size && y < 2 * halfpatch_size) {
    ...
    *patch_ptr = wTL * img_ptr[0];
    ...
    if (within_border) {
      float* patch_wo_border_ptr = d_patch + y_wo_border * 2 * halfpatch_size + x_wo_border;
